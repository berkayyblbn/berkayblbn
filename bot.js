...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {
    msg.reply('Sunucuya Hoşgeldin Aleyküm Selam !');
  }
});
...
client.on('message', async message => {
  ...
  return message.channel.send(" Bu Kodu `Yönetici` Yetkisi Olan Kullanabilir");
   ...
   message.guild.createChannel('Writing Channels', 'category', [{
  id: message.guild.id,
  deny: ['SEND_MESSAGES']
}])
...
.then(channel =>
 channel.setParent(message.guild.channels.find(channel => channel.name === "Writing Channels")));
...
.then(channel =>
             channel.setParent(message.guild.channels.find(channel => channel.name === "Writing Channels")));
             message.guild.createChannel('öneri', 'text', [{
              id: message.guild.id,
              deny: ['SEND_MESSAGES']
            }])
            .then(channel => channel.setParent(message.guild.channels.find(channel => channel.name === "Writing Channels")));
            message.guild.createChannel('social-media', 'text', [{
              id: message.guild.id,
              deny: ['SEND_MESSAGES']
            }])
.then(channel =>
 channel.setParent(message.guild.channels.find(channel => channel.name === "Writing Channels")));
                    ...
                    message.guild.createChannel('level-log', 'text', [{
              id: message.guild.id,
              deny: ['SEND_MESSAGES']
            }])
        .then(channel => channel.setParent(message.guild.channels.find(channel => channel.name === "Writing Channels")));
            message.guild.createChannel('oyun-seç****', 'text', [{
              id: message.guild.id,
              deny: ['SEND_MESSAGES']
               }])
 ...
 channel.setParent(message.guild.channels.find(channel => channel.name === "✮ ▬  ▬ Duyuru Kanalları▬  ▬ ✮")));
  ...
  .then(c => {
    ...
    c.overwritePermissions(role, {
       ...
       message.guild.owner.send("Gerekli Herşey Kuruldu Rahatına Bak! **Anathema Bot'un kıymetini bil :D**")
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    let embed = new Discord.RichEmbed()
                    .setColor(0xffa300)
                    .setFooter('Anathema Bot  -|-  Reklam engellendi.', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("Anathema Reklam sistemi, " + `***${msg.guild.name}***` + " adlı sunucunuzda reklam yakaladım.")
...
if (!modlogkanal) return;
	if (channel.type === "text") {
				...
				.setFooter(`,Anathema BOT | Mod-Log Sistemi Kanal ID: ${channel.id}`)
...
.setTitle("SES KANALI OLUŞTURULDU")
				...
				.setFooter(`Anathema | Mod-Log Sistemi Kanal ID: ${channel.id}`)
...
.setTitle("SES KANALI SİLİNDİ")
			...
			.setFooter(`AnathemaBOT | Mod-Log Sistemi  Kanal ID: ${channel.id}`)
				modlogkanal.send({embed});
			}
	})
...
.then(channel =>
             channel.setParent(message.guild.channels.find(channel => channel.name === "Server Panel")));
message.guild.createChannel(`Anathema Bot • Sunar !`, 'voice')
.then(channel =>
 channel.setParent(message.guild.channels.find(channel => channel.name === "Server Panel")));
